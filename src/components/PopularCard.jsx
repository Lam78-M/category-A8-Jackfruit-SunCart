...
import homeside from '@/data/homesid.json'
  ...
  style={{
    ...
    borderRadius: "16px"
             ...
             <div className=" grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5  gap-6">
                    {homeside.homeside.map(home => (
                      <div key={home.id} className="bg-white rounded-2xl shadow-sm border border-amber-100 overflow-hidden flex flex-col hover:shadow-md transition-shadow">
              ...
              className="object-cover rounded-t-2xl"
                        ...
                        <div className="p-4 flex flex-col gap-2 flex-1">
                          {/* Name & Brand */}
                          <h2 className=" text-2xl font-semibold text-gray-800">{home.name}</h2>
                         
                          {/* Rating */}
                          <p className="text-sm text-amber-500 flex items-center gap-2 pt-3"><FaRegStar className='text-lg' /> {home.rating} / 5</p>
            
                          {/* Price + Button */}
                          <div className="mt-auto flex items-center justify-between pt-3 border-t border-amber-100">
                            <span className="text-lg font-bold text-amber-700">৳{home.price}</span>
